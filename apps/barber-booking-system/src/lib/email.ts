...
export async function sendCancellationEmail({
  ...
  const refundNote = refunded && refundAmount
    ? `<p style="color:#4a8800;font-size:14px;margin:12px 0 0;">A refund of <strong>$${refundAmount.toFixed(2)}</strong> has been issued to your original payment method.</p>`
    : `<p style="color:#888;font-size:13px;margin:12px 0 0;">Your deposit was forfeited per our cancellation policy (cancellations within 24 hours of the appointment).</p>`;
...
<body style="background:#050505;color:#ffffff;font-family:system-ui,sans-serif;margin:0;padding:0;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#050505;padding:40px 20px;">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#0d0d0d;border:1px solid #1f1f1f;border-radius:16px;overflow:hidden;">
        <tr><td style="background:#444;height:4px;"></td></tr>
        <tr><td style="padding:32px 36px;">
          <div style="font-size:36px;margin-bottom:16px;">❌</div>
          <h1 style="color:#ffffff;font-size:24px;font-weight:900;margin:0 0 6px;">Appointment Cancelled</h1>
          <p style="color:#888;font-size:14px;margin:0 0 28px;">${shopName}</p>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;margin-bottom:16px;">
            <tr><td style="padding:20px 24px;">
              <p style="margin:0 0 4px;font-weight:800;font-size:16px;">${customerName}</p>
              <p style="margin:0 0 4px;color:#aaa;font-size:14px;">${serviceName} with ${barberName}</p>
              <p style="margin:0 0 4px;color:#666;font-size:14px;text-decoration:line-through;">${dateStr} at ${timeStr}</p>
              <p style="margin:0;color:#555;font-size:13px;">Booking Code: <strong style="color:#fff;font-family:monospace;">${bookingCode}</strong></p>
            </td></tr>
          </table>
          ${refundNote}
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "SquareBidness Booking <bookings@squarebidness.com>",
      to: [to],
      subject: `Cancelled: ${serviceName} with ${barberName} — ${dateStr}`,
      html,
    }),
